...
```{r}
predictions_ss <- ss_simulate(ref_table = results$ss_models_info, models = results$ss_models,
                              extrapolate = c(0,1))
head(predictions_ss)
```
...
Alternatively, the `urbantrees` data can be fit to a linear mixed-effects model with 'species' specified as the random effect, using the `lme4::lmer` function under the hood.  
...
Likewise, we can simulate data across a range of diameter sizes for each species, and make predictions of tree height. Extrapolation is also possible using the argument `extrapolate`:
```{r warning = FALSE, message = FALSE}
...
```{r single-species_model curves, message = FALSE, warning = FALSE, fig.width=6.5, fig.height =4, fig.align='center', echo = FALSE}
...
ggplot() +
  ...
  # prediction interval
  geom_ribbon(data = predictions_ss, aes(x = predictor, ymin = lwr, ymax = upr), 
              alpha = 0.10) +
  
  # regression line
  geom_line(data = predictions_ss, aes(x = predictor, y = fit, lty = extrapolated),
            size = 0.8, alpha = 0.6) +
...
```{r mixed-effects model curves, fig.width = 6.5, fig.height = 4, fig.align = 'center', echo = FALSE}
...
ggplot() +
  ...
  # prediction interval
  geom_ribbon(data = predictions_mix, aes(x = predictor, ymin = lwr, ymax = upr), 
              alpha = 0.10) +
  
  # regression line
  geom_line(data = predictions_mix, aes(x = predictor, y = fit, lty = extrapolated),
            size = 0.8, alpha = 0.6) +
  ...
  coord_cartesian(ylim=c(0, 25), xlim= c(0,max(predictions_mix$predictor)))
```
