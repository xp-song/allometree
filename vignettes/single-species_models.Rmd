...
Lets use this threshold across all species in `urbantrees`. We can create a function `uncooker()` that removes outliers for each species using a `for()` loop, based on their respective best-fit model in `results_all$sp_models`:
...
ggplot() +
  facet_wrap(~ species)+ 
  geom_point(data = dplyr::filter(urbantrees, species == unique(predictions$species)), aes(x = diameter, y = height),
             size=0.35, alpha = 0.2, color = "grey70") +
