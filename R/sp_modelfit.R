#' Fit data to a pre-selected model for one species
#'
#' Fit data to a pre-selected allometric equation, for one species.
#' Allometric equations that may be considered as an input to this function
#' can be found in `?eqns_info` and `data(eqns_info)`.
#'
...
#'
#' ## fitted_model_info
#' More information on output columns:
#' * `modelcode`: Model code for the best-fit equation
#' * `a`, `b`, `c`, `d`, `e`: Parameter estimates
#' * `response_geom_mean`: Geometric mean of the response variable used in
#' calculation of AICc (only for transformed models)
#' * `correctn_factor`: Bias correction factor to use on model predictions
#' (only for transformed models)
#' * `predictor_min`, `predictor_max`: Range of the predictor variable within
...
#' * `adj_R2`: Adjusted \eqn{R^2} of the model
...
#' @seealso [sp_modelfit_multi()] to fit pre-selected models across multiple
#'   species.
#'
...
#' results <- sp_modelfit(Alb_sam, modelcode = 'quad_w1', # manually specify equation to use
...
#' Fit data to pre-selected models across multiple species
#'
#' Wrapper to run `sp_modelfit()` across multiple species. Data is fit to
...
#'   2. Table showing each species' resulting model information, named
#'   `sp_models_info`.
#'
#' ## sp_models_info
#' More information on output columns:
...
#'
#' @family single-species model functions
#' @seealso [sp_modelfit()] to fit pre-selected model for one species.
#'
#'   [sp_modelselect()] to select a best-fit model for one species.
#'
#'   [sp_modelselect_multi()] to select best-fit models across multiple species.
