...
sp_simulate <- function(ref_table, models, select_sp = NULL, level = 0.95, extrapolate = NULL, species = "species",
    ...
    predict_range_full <- tidyr::pivot_longer(predict_range_full, cols = colnames(predict_range_full), names_to = "species",
        values_to = "predictor")
    ...
    if (!is.null(extrapolate)) {
        ...
        predict_range_low <- apply(predict_range, 1, function(x) if(x[predictor_min] > extrapolate[1]){seq(0, x[predictor_min], length.out = 20)}else{rep(NA, 100)})  #each sp is a column
        ...
        predict_range_high <- apply(predict_range, 1, function(x) if(x[predictor_max] < extrapolate[2]){seq(extrapolate[2], x[predictor_max], length.out = 100)}else{rep(NA, 100)})  #each sp is a column
