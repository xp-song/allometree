...
#' `?eqns_info` and `data(eqns_info)`. To make the AICc values of equations with a transformed
...
#'   AIC and BIC in model selection. *Sociological Methods and Research*, **33**(2),
#'   261–304.
#'
...
#'
#' @param data Dataframe that contains the variables of interest. Each row is a measurement
#' for an individual tree of a particular species.
#' @param species Column name of the species variable. Defaults to `species`.
#' @param response Column name of the response variable. Defaults to `height`.
#' @param predictor Column name of the predictor variable. Defaults to `diameter`.
#'
#' @return
#' A list of 3 elements:
#'   1. List of tables showing each species' candidate models ranked by AICc value,
#'   named `sp_models_rank`.
...
#'
#' head(results$sp_models_rank[[1]]) # Highly-ranked models for first species in list
#'
